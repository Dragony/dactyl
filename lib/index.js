...
var Start = function(configPath, cwd){
    ...
    for(name in config.routing){
        ...
        if(element.type){
            app[element.type](element.path, router.bind(router, controllerFunc));
        }else{
    ...
    io.sockets.on('connection', function (socket){
        if(socket.storage.sockets == undefined){
            socket.storage.sockets = [];
        }
        socket.storage.sockets.push(socket);
